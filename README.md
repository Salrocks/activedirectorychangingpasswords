...
<ul>
<li>After accessing the find function, we will enter the username we wish to locate. In this instance, I am searching for 'Robert'. Upon clicking 'Find now', we receive the search results.</li><br></ul>
...
<ul>
<li>After obtaining the results, double-clicking on the name will open a tab similar to the one below. Within this tab, various options are presented, but our primary focus is on the 'Account' section, so let's navigate there.</li><br></ul>
...
<ul>
<li>On the account page, our attention is drawn to the section labeled 'Account Options' in the middle. Here, we must ensure to select the option 'User must change password on next logon,' which will compel the user to create a new password upon their next login. Once this option is checked, proceed to click 'OK' at the bottom.</li><br></ul>
...
<ul>
<li>After opting to reset the password, we are presented with this tab. Here, we will generate a new temporary password, which we will then share with Robert. Once the new temporary password is created, we click 'OK' to proceed.</li><br></ul>
...
<ul>
<li>In the image below, it's evident that we have successfully reset the password for the user Robert. Remember, once Robert logs in with the temporary password we provide, he will be prompted to create a new password.</li><br></ul>
